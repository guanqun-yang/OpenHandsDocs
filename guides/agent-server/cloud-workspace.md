...
5. Store this key securely and use it as the `OPENHANDS_CLOUD_API_KEY` environment variable.
...
Usage: uv run examples/02_remote_agent_server/06_convo_with_cloud_workspace.py
...
Note: The LLM configuration is sent to the cloud sandbox, so you need an API
key that works directly with the LLM provider (not a local proxy). If using
...
- [API-based Sandbox](api-sandbox.md) - Connect to Runtime API service
- [Docker Sandboxed Server](docker-sandbox.md) - Run locally with Docker
- [Local Agent Server](local-server.md) - Development without containers
