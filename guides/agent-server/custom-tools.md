...
A ready-to-run example is available here! When using a remote agent server, custom tools must be available in the server's Python environment. This guide shows how to build a custom base image with your tools and use `DockerDevWorkspace` to automatically build the agent server on top of it.

For standalone custom tools (without remote agent server), see the [Custom Tools guide](../custom-tools.md).
...
class LogDataExecutor(ToolExecutor[LogDataAction, LogDataObservation]):
    ...
    def __call__(
        self, action: LogDataAction, conversation=None  # noqa: ARG002
    ) -> LogDataObservation:
...
This example shows how to use custom tools with a remote agent server:
...
- [Custom Tools (Standalone)](../custom-tools.md) - For local execution without remote server
- [Agent Server Overview](overview.md) - Understanding remote agent servers
