...
The LLM you use must support image inputs (`llm.vision_is_active()` need to be True). Pass images along with text in the message content:
...
This script mirrors the basic setup from ``examples/01_hello_world.py``
but adds vision support by sending an image to the agent alongside text instructions.
"""
