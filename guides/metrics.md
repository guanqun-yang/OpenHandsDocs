...
The OpenHands SDK provides metrics tracking at two levels:

- Individual LLM metrics
- Aggregated conversation-level costs
...
Each LLM is created with a unique `usage_id` (e.g., "agent", "condenser") that serves as its identifier in the registry.

The registry maintains references to all LLM instances, allowing you to:
